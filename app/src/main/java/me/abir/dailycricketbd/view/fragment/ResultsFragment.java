...
import me.abir.dailycricketbd.viewmodel.ResultsViewModel;
...
public class ResultsFragment extends Fragment {
    ...
    private void initView(View view) {
        pbResults = view.findViewById(R.id.pbResults);
        rvResults = view.findViewById(R.id.rvResults);
        rvResults.setLayoutManager(new LinearLayoutManager(context));
        /*resultsAdapter = new ResultsAdapter();
        rvResults.setAdapter(resultsAdapter);*/
        //rvResults.addItemDecoration(setStickyHeader());
   ...
   /* private RecyclerSectionItemDecoration setStickyHeader() {
    ...
    private void getResultsFromApi() {
        ResultsViewModel viewModel = ViewModelProviders.of(getActivity()).get(ResultsViewModel.class);
        viewModel.getResults(CricUtil.getAccessToken(context)).observe(this,
                new Observer<List<ResultsGroupChildModel>>() {
                    @Override
                    public void onChanged(
                            @Nullable List<ResultsGroupChildModel> resultsModels) {
                        if (resultsModels != null && resultsModels.size() > 0) {
                            //this.resultsModels = resultsModels;
                            //resultsAdapter.setData(resultsModels, false);
                            resultsAdapter = new ResultsAdapter(resultsModels, false);
